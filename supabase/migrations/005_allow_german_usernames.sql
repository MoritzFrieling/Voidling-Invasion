-- Allow German umlauts and sharp S in player callsigns alongside Hangul.
...
alter table public.profiles
  ...
  check (
    ...
    or (username ~ '^[a-z0-9_äöüß가-힣]+$' and username ~ '[äöüß가-힣]' and length(username) between 2 and 20)
  );
...
begin
  ...
  if not (
    ...
    or (requested_username ~ '^[a-z0-9_äöüß가-힣]+$' and requested_username ~ '[äöüß가-힣]' and length(requested_username) between 2 and 20)
  ) then
...
begin
  ...
  if not (
    ...
    or (normalized ~ '^[a-z0-9_äöüß가-힣]+$' and normalized ~ '[äöüß가-힣]' and length(normalized) between 2 and 20)
